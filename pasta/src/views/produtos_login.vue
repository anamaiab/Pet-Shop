      ...
      for (var i = 0; i < dados.length; i++) {
        if (dados[i].estoque > 0) {
          ...
          b.onclick = function() {
            ...
            var r = confirm(
              "Você adicionará esse produto ao carrinho. Deseja continuar?"
            );
            ...
            if (r == false) return;
              ...
              fetch("http://localhost:3000/products/id/" + idBotao.id, {
                method: "GET",
                headers: {
                  Accept: "application/json",
                  "Content-Type": "application/json"
                ...
                if (res.ok) {
                  res
                    .json()
                    .then(jsonRes => {
                      var name = jsonRes.name;
                      var price = jsonRes.price;
                      var photo = jsonRes.photo;
                      var id_p = jsonRes._id;

                      //Pegando o id de quem esta logado no momento
                      fetch("http://localhost:3000/user/logged", {
                        ...
                        if (res.ok) {
                          res
                            .json()
                            .then(jsonRes => {
                              var id = jsonRes[0]._id;

                              //Adiciona no carrinho com todas as informações recolhidas anterirmente
